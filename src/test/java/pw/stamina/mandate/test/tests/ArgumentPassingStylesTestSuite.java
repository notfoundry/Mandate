 ...
 * @author Mark Johnson
 */
public class ArgumentPassingStylesTestSuite {
    ...
    @Test
    public void testUsingUserFlagsWithAndWithoutInlineEqualsSign() {
        final Execution result = commandContext.execute("run -f \"top=kek lel\" --bar=l=e=l -quz=x");

        assertTrue(result.result() == ExitCode.SUCCESS);

        assertEquals(3, commandOutput.size());

        assertEquals(0, commandErrors.size());

        assertEquals("top=kek lel", commandOutput.poll());

        assertEquals("l=e=l", commandOutput.poll());

        assertEquals("x", commandOutput.poll());
    }

    @Executes
    @Syntax(root = "run")
    public ExitCode accept(@Implicit final IODescriptor io,
                           @UserFlag(flag = "f") final String fooInput,
                           @UserFlag(flag = "-bar") final String barInput,
                           @UserFlag(flag = "quz") final String quzInput) {
        io.out().write(fooInput);
        io.out().write(barInput);
        io.out().write(quzInput);
